...
namespace Gear.ActiveQuery.Tests
{
    ...
    public class EnumerableRangeActiveExpression
    {
        ...
        public void GenericDisposalOverridden()
        {
            ...
            using (var queryA = people.ActiveSelect(person => 3 / (person as TestPerson).Name.Length))
            using (var queryB = people.ActiveSelect(person => 3 / (person as TestPerson).Name.Length))
                people.Clear();
        }
